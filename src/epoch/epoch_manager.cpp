...
//            while(!EpochMessageReceiveHandler::IsRedoLogPushDownACKReceiveComplete(i)) usleep(logical_sleep_timme);
            ...
            while(i >= commit_epoch.load()) usleep(sleep_time);
            while(!EpochManager::IsCommitComplete(i)) usleep(sleep_time);
            while(!RedoLoger::CheckPushDownComplete(i)) usleep(sleep_time);
            EpochMessageSendHandler::SendTxnToServer(i,i, empty_txn_ptr, proto::TxnType::EpochLogPushDownComplete);
            while(!EpochMessageReceiveHandler::IsRedoLogPushDownACKReceiveComplete(i)) usleep(sleep_time);
